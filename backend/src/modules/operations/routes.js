...
export function createOperationsRouter({ pool, environment }) {
  const router = Router();
  const service = createOperationsService({ pool, environment });
  ...
  router.post("/admin/operation-cases/:caseId/actions", requireRole("ADMIN"), asyncHandler(async (request, response) => {
    const input = parse(z.object({
      action: z.enum(["RESOLVE", "DISMISS", "ASSIGN", "RETRY", "RETRY_AI_JOB", "DISABLE_FEATURE", "APPROVE_LATE_CHANGE", "CONFIRM_NO_SHOW", "PUBLISH_REVIEW", "HIDE_REVIEW", "UPHOLD_DISPUTE", "DISMISS_DISPUTE"]),
