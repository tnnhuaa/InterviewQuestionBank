...
export default function ErrorPanel({
  ...
  return (
    <div
      role="alert"
      className="rounded-xl border border-danger/20 bg-danger-soft p-5 text-sm text-ink"
    >
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-semibold">{apiError?.message ?? "Đã có lỗi."}</p>
          <p className="mt-1 text-xs text-ink-secondary">
            {RECOVERY_LABELS[apiError?.recovery.kind ?? "NONE"]}
          </p>
          {apiError && Object.keys(apiError.fieldErrors).length > 0 ? (
            <ul className="mt-3 space-y-1 rounded-md border border-danger/15 bg-panel/70 p-3">
              {Object.entries(apiError.fieldErrors).map(([field, message]) => (
                <li key={field} className="text-xs text-ink-secondary">
                  <span className="font-semibold text-danger">{field}:</span>{" "}
                  {message}
                </li>
              ))}
            </ul>
          ) : null}
          {apiError?.status === 429 && apiError.recovery.retryAfterSeconds && (
            <p className="mt-1 text-xs text-ink-secondary">
              Thử lại sau {apiError.recovery.retryAfterSeconds} giây.
            </p>
          )}
          <div className="mt-3 flex flex-wrap gap-2">
            {onRetry && apiError?.recovery.retryable && (
              <button
                type="button"
                onClick={onRetry}
            ...
            <button
              type="button"
              onClick={() => navigator.clipboard.writeText(safeDetails)}
              className="inline-flex items-center gap-1 rounded-md border border-edge bg-panel px-3 py-1.5 text-xs font-medium text-ink-secondary"
            >
              <Copy aria-hidden size={13} /> Sao chép thông tin hỗ trợ
            </button>
          </div>
          {apiError?.correlationId && (
            <p className="mt-3 break-all font-mono text-[11px] text-ink-muted">
