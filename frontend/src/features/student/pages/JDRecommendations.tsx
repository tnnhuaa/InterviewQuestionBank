...
function MatchSelection({ jobDescriptionId, analysisVersion }: { jobDescriptionId: string; analysisVersion?: number }) {
  ...
  if (!matches.data?.matches.length) return <div className="rounded-xl border border-edge bg-panel p-8 text-center"><p className="text-sm font-semibold text-ink">Chưa có câu hỏi đạt mức phù hợp tối thiểu 60 điểm</p><p className="mt-1 text-xs text-ink-muted">Các đoạn trích và yêu cầu đã nhận diện vẫn được giữ lại. Bạn có thể tìm câu hỏi thủ công.</p><Link to="/questions?from=jd" className="mt-4 inline-block text-sm font-medium text-primary">Mở ngân hàng câu hỏi</Link></div>;
