...
const transitionSchema = z.object({
  ...
  proposedSlotId: z.uuid().optional(),
...
export function createBookingsRouter({ pool, environment }) {
  ...
  router.post("/bookings", requireRole("STUDENT"), asyncHandler(async (request, response) => {
    const input = parse(z.object({
      mentorId: z.uuid(), slotId: z.uuid(), jobDescriptionId: z.uuid().optional(), preparationPlanId: z.uuid().optional(),
  ...
  router.post("/bookings/:bookingId/feedback", requireRole("MENTOR"), asyncHandler(async (request, response) => {
    const input = parse(z.object({
      rubricScores: z.record(z.string(), z.number().min(0).max(5)), strengths: z.string().trim().min(10).max(5000),
      weaknesses: z.string().trim().min(10).max(5000), nextActions: z.array(z.string().trim().min(3).max(500)).min(1).max(20),
    }), request.body);
  ...
  router.post("/bookings/:bookingId/feedback/apply", requireRole("STUDENT"), asyncHandler(async (request, response) => {
    const input = parse(z.object({ actions: z.array(z.string()).min(1).max(20) }), request.body);
