...
const verificationSubmissionSchema = z.object({
  consent: z.literal("true", {
    message: "Cần xác nhận đồng ý xử lý bằng chứng xác minh",
  }),
...
export function createMentorsRouter({ pool, storage, environment }) {
    ...
    asyncHandler(async (request, response) => {
      response.json(
        await service.saveProfile(
          ...
          parse(profileSchema, request.body),
        ),
      );
    }),
  );
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      const input = parse(
        z
          ...
          .refine((value) => new Date(value.startsAt) > new Date(), {
            message: "Khung giờ phải ở tương lai",
            path: ["startsAt"],
          }),
        request.body,
      );
      response
        .status(201)
        .json(await service.createSlot(request.auth.user.id, input));
    }),
  );
  ...
  router.get(
    ...
    asyncHandler(async (request, response) => {
      response.json(
        await service.getVerificationForReview(request.params.verificationId),
      );
    }),
  );
  ...
  router.get(
    ...
    requireRole("ADMIN"),
    asyncHandler(async (request, response) => {
      await service.verifyEvidenceExists(request.params.verificationId);
      const access = service.createEvidenceAccess(
